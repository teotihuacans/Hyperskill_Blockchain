...
public class VerifyMessage {
    ...
    public VerifyMessage(List<byte[]> list, byte[] keyBytes) throws Exception {
        /*ObjectInputStream in = new ObjectInputStream(new FileInputStream(filename));
        this.list = (List<byte[]>) in.readObject();
        in.close();*/
        this.list = list;

        System.out.println(verifySignature(list.get(0), list.get(1), keyBytes) ?
    ...
    private boolean verifySignature(byte[] data, byte[] signature, byte[] keyBytes) throws Exception {
        Signature sig = Signature.getInstance("SHA1withRSA");
        sig.initVerify(getPublic(keyBytes));
        sig.update(data);
